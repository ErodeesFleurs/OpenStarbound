...
local function updateVolumeCanvas(canvas, volume, multiplier)
    ...
    local str = volume == 0 and str("^#f00,shadow;", "Muted") or
        fmt("^shadow;%s%%", math.floor(volume * multiplier * 1000) * 0.1)
    canvas:drawText(str, { position = { 92.5, MID }, horizontalAnchor = "mid", verticalAnchor = "mid" }, 16,
        { 255, 255, 255, 255 })
end
...
local function updateThresholdCanvas(canvas, dB)
    ...
    local str_ = fmt("^shadow;%sdB", math.floor(dB * 10) * 0.1)
    canvas:drawText(str_, { position = { 92.5, MID }, horizontalAnchor = "mid", verticalAnchor = "mid" }, 16,
        { 255, 255, 255, 255 })
end
