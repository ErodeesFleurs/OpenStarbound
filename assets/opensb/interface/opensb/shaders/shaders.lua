...
local function updateParameter(parameter, oname, value)
    if parameter.isPasses then
        for _, v in next, parameter.layers do
            renderer.setPostProcessLayerPasses(v, value)
        end
    else
        for _, v in next, parameter.effects do
