...
for _, this_path in ipairs(assets.scan("/humanoid/", "cosmetic.animation")) do
    for i = 1, 20 do
        assets.add(this_path .. "." .. i, assets.bytes(this_path):gsub(
            '<slot>', tostring(i)
        ):gsub(
            ...
            '"flippedZLevelChestLegs"%s*:%s*(%d+%.%d+)',
            '"flippedZLevel":%1' .. string.format('%04d', chestLegsSortOrder[i])
        ):gsub(
        ...
        ---@diagnostic disable-next-line: need-check-nil
            ...
            '"flippedZLevelChestLegs"%s*:%s*(%-%d+%.%d+)',
            ---@diagnostic disable-next-line: need-check-nil
            '"flippedZLevel":%1' .. string.format('%04d', 20 - chestLegsSortOrder[i])
        ))
    end
end
