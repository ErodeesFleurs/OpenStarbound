...
local function line(pos, value)
    ...
    ---@diagnostic disable-next-line: preferred-local-alias, assign-type-mismatch
    LINE_COLOR[4] = 255 * math.min(1, sqrt(width / 350))
    if width > 0 then
        --- @cast canvas CanvasWidget
        --- @cast linePaddingDrawable function
        canvas:drawDrawable(linePaddingDrawable(0, min(12, width)), pos)
        if width > 12 then
            lineDrawable.line[2][1] = min(width, LINE_WIDTH_PADDED)
...
local function drawSpeakerBar(mouse, pos, speaker, i)
    ...
    if hovering then
        ...
        if input.keyHeld("LShift") then
            ...
            canvas:drawText("^#fff7,font=iosevka-semibold;" .. tostring(speaker.speakerId), textPositioning, 16, nil, nil,
                nil, FONT_DIRECTIVES)
        end
        ...
        if input.mouseDown("MouseLeft") then
            ...
            interface.queueMessage((muted and "^#f43030;Muted^reset; " or "^#31d2f7;Unmuted^reset; ") .. speaker.name, 4,
                0.5)
