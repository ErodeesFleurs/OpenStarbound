...
void CanvasWidget::renderImpl() {
  ...
  for (auto const& op : m_renderOps) {
    ...
    else if (auto args = op.ptr<ImageRectOp>())
    ...
    else if (auto args = op.ptr<DrawableOp>())
    ...
    else if (auto args = op.ptr<TiledImageOp>())
    ...
    else if (auto args = op.ptr<TextOp>())
