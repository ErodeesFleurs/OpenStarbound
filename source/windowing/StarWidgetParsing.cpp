...
GuiContext& WidgetParser::guiContext() const {
  return m_context;
}
...
List<WidgetConstructResult> WidgetParser::constructor(Json const& config) {
  ...
  auto addWidget = [this, &widgets](Json const& memberConfig) {
    ...
    if (type == "include") {
      widgets.appendAll(constructor(guiContext().assets()->json(memberConfig.getString("file"))));
    } else {
...
WidgetConstructResult WidgetParser::tabSetHandler(String const& name, Json const& config) {
  ...
  Json defaultPressedOffset = guiContext().assets()->json("/interface.config:buttonPressedOffset");
