...
std::flat_map<std::string, color> const color::named_colors{
  {"red", color::Red},
  {"orange", color::Orange},
  {"yellow", color::Yellow},
  {"green", color::Green},
...
constexpr auto color::temperature(float temp) -> color {
    ...
    if (temp <= 66) {
        ...
        b = (temp <= 19)
          ? 0
          : std::clamp(138.5177312231 * std::log(temp - 10) - 305.0447927307, 0.0, 255.0);
    } else {
    ...
    return rgbaf(static_cast<float>(r) / 255.0F, static_cast<float>(g) / 255.0F,
...
color::color(std::u8string_view name) {
    if (name.starts_with('#')) {
        *this = from_hex(name.substr(1));
    } else {
        auto i = named_colors.find(std::string(name.begin(), name.end()));
        if (i != named_colors.end()) {
            *this = i->second;
        } else {
            throw color_exception::format("Named color {} not found", to_sv(name));
        }
    }
}

constexpr auto color::from_hex(std::u8string_view s) -> color {
    return color::rgba(hex_to_vec_4b(s));
}
...
constexpr auto color::to_hex() const -> std::u8string {
    auto rgba = to_rgba();
...
constexpr auto color::multiply(float amount) const -> color {
...
auto color::operator*(color const& c) const -> color {
    return color::rgbaf(m_data.piecewise_multiply(c.to_rgbaf()));
}
...
auto color::hex_to_vec_4b(std::u8string_view s) -> vec_4b {
