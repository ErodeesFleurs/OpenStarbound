...
Vec4B Color::hexToVec4B(StringView s) {
  ...
  if (s.utf8Size() == 3) {
    [[maybe_unused]] size_t decoded = nibbleDecode(s.utf8Ptr(), 3, reinterpret_cast<char*>(cbytes.data()), 4);
    assert(decoded == 3);
    ...
    [[maybe_unused]] size_t decoded = nibbleDecode(s.utf8Ptr(), 4, reinterpret_cast<char*>(cbytes.data()), 4);
    assert(decoded == 4);
    cbytes[0] = (cbytes[0] << 4) | cbytes[0];
    cbytes[1] = (cbytes[1] << 4) | cbytes[1];
    cbytes[2] = (cbytes[2] << 4) | cbytes[2];
    ...
    [[maybe_unused]] size_t decoded = hexDecode(s.utf8Ptr(), 6, reinterpret_cast<char*>(cbytes.data()), 4);
    assert(decoded == 3);
    cbytes[3] = 255;
  } else if (s.utf8Size() == 8) {
    [[maybe_unused]] size_t decoded = hexDecode(s.utf8Ptr(), 8, reinterpret_cast<char*>(cbytes.data()), 4);
    assert(decoded == 4);
  } else {
