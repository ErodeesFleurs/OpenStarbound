...
struct MultiArrayInterpolator2 {
	...
	using IndexList = std::array<std::size_t, Rank>;
	using SizeList = std::array<std::size_t, Rank>;
	using PositionList = std::array<Position, Rank>;
	using WeightList = std::array<Position, 2>;
	...
	auto interpolate(MultiArray const& array, PositionList const& coord) const -> Element {
		...
		for (std::size_t i = 0; i < Rank; ++i) {
			auto binfo = get_bound2(coord[i], array.size(i), boundMode);
			imin[i] = binfo.i0;
			imax[i] = binfo.i1;
...
struct MultiArrayInterpolator4 {
	...
	using SizeList = std::array<std::size_t, Rank>;
	using PositionList = std::array<Position, Rank>;
	using WeightList = std::array<Position, 4>;
...
struct MultiArrayPiecewiseInterpolator {
	...
	using PiecewiseRangeList = std::array<PiecewiseRange, Rank>;
	...
	auto interpolate(MultiArray const& array, PositionList const& coord) const -> Element {
		...
		for (std::size_t i = 0; i < Rank; ++i) {
			...
			auto bound = get_bound2(coord[i], array.size(i), boundMode);
			minIndex[i] = bound.i0;
			maxIndex[i] = bound.i1;
