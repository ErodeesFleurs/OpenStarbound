...
public:
  ...
  [[nodiscard]] static String convertDirSeparators(String const& path);
  ...
  [[nodiscard]] static List<pair<String, bool>> dirList(String const& dirName, bool skipDots = true);

  // Returns the final component of the given path with no directory separators
  [[nodiscard]] static String baseName(String const& fileName);
  // All components of the given path minus the final component, separated by
  // the directory separator
  [[nodiscard]] static String dirName(String const& fileName);

  // Resolve a path relative to another path.  If the given path is absolute,
  // then the given path is returned unmodified.
  [[nodiscard]] static String relativeTo(String const& relativeTo, String const& path);

  // Resolve the given possibly relative path into an absolute path.
  [[nodiscard]] static String fullPath(String const& path);

  [[nodiscard]] static String temporaryFileName();
  ...
  [[nodiscard]] static FilePtr temporaryFile();

  // Creates and opens new ReadWrite temporary file and opens it.  This file
  // has no filename and will be removed on close.
  [[nodiscard]] static FilePtr ephemeralFile();

  // Creates a new temporary directory and returns the path.  Will not be
  // removed automatically.
  [[nodiscard]] static String temporaryDirectory();

  [[nodiscard]] static bool exists(String const& path);

  // Does the file exist and is it a regular file (not a directory or special
  // file)?
  [[nodiscard]] static bool isFile(String const& path);
  // Is the file a directory?
  [[nodiscard]] static bool isDirectory(String const& path);
  ...
  [[nodiscard]] static ByteArray readFile(String const& filename);
  [[nodiscard]] static String readFileString(String const& filename);
  [[nodiscard]] static StreamOffset fileSize(String const& filename);
  ...
  [[nodiscard]] static FilePtr open(String const& filename, IOMode mode);
...
private:
  [[nodiscard]] static void* fopen(char const* filename, IOMode mode);
  static void fseek(void* file, StreamOffset offset, IOSeek seek);
  [[nodiscard]] static StreamOffset ftell(void* file);
  [[nodiscard]] static size_t fread(void* file, char* data, size_t len);
  [[nodiscard]] static size_t fwrite(void* file, char const* data, size_t len);
  static void fsync(void* file);
  static void fclose(void* file);
  [[nodiscard]] static StreamOffset fsize(void* file);
  [[nodiscard]] static size_t pread(void* file, char* data, size_t len, StreamOffset absPosition);
  [[nodiscard]] static size_t pwrite(void* file, char const* data, size_t len, StreamOffset absPosition);
