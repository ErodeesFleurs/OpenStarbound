...
struct FramesSpecification {
  ...
  [[nodiscard]] Maybe<RectU> getRect(String const& frame) const;
  // Converts to Json.
...
public:
  ...
  struct AssetId {
    ...
    [[nodiscard]] bool operator==(AssetId const& assetId) const;
  };

  struct AssetIdHash {
    [[nodiscard]] size_t operator()(AssetId const& id) const;
  };
  ...
  struct AssetData {
    ...
    // the cache.
    [[nodiscard]] virtual bool shouldPersist() const = 0;
  ...
  struct JsonData : AssetData {
    [[nodiscard]] bool shouldPersist() const;
  ...
  [[nodiscard]] StringList assetSources() const;

  // Return metadata for the given loaded asset source path
  [[nodiscard]] JsonObject assetSourceMetadata(String const& sourcePath) const;
  ...
  [[nodiscard]] ByteArray digest() const;

  // Is there an asset associated with the given path?  Path must not contain
  // sub-paths or directives.
  [[nodiscard]] bool assetExists(String const& path) const;

  [[nodiscard]] Maybe<AssetFileDescriptor> assetDescriptor(String const& path) const;

  // The name of the asset source within which the path exists.
  [[nodiscard]] String assetSource(String const& path) const;

  [[nodiscard]] Maybe<String> assetSourcePath(AssetSourcePtr const& source) const;

  // Scans for all assets with the given suffix in any directory.
  [[nodiscard]] StringList scan(String const& suffix) const;
  // Scans for all assets matching both prefix and suffix (prefix may be, for
  // example, a directory)
  [[nodiscard]] StringList scan(String const& prefix, String const& suffix) const;
  ...
  [[nodiscard]] CaseInsensitiveStringSet const& scanExtension(String const& extension) const;
  ...
  [[nodiscard]] ImageConstPtr image(AssetPath const& path) const;
  ...
  // loading.
  [[nodiscard]] ImageConstPtr tryImage(AssetPath const& path) const;
  ...
  // given image path.
  [[nodiscard]] FramesSpecificationConstPtr imageFrames(String const& path) const;

  // Returns a pointer to a shared audio asset;
  [[nodiscard]] AudioConstPtr audio(String const& path) const;
  ...
  // loading.
  [[nodiscard]] AudioConstPtr tryAudio(String const& path) const;

  // Returns pointer to shared font asset
  [[nodiscard]] FontConstPtr font(String const& path) const;

  // Returns a bytes asset (Reads asset as an opaque binary blob)
  [[nodiscard]] ByteArrayConstPtr bytes(String const& path) const;

  // Bypass asset caching and open an asset file directly.
  [[nodiscard]] IODevicePtr openFile(String const& basePath) const;
...
private:
  ...
  [[nodiscard]] static FramesSpecification parseFramesSpecification(Json const& frameConfig, String path);

  void queueAssets(List<AssetId> const& assetIds) const;
  //Lock before calling!
  void queueAsset(AssetId const& assetId) const;
  [[nodiscard]] shared_ptr<AssetData> tryAsset(AssetId const& id) const;
  [[nodiscard]] shared_ptr<AssetData> getAsset(AssetId const& id) const;
  ...
  [[nodiscard]] FramesSpecificationConstPtr bestFramesSpecification(String const& basePath) const;

  [[nodiscard]] IODevicePtr open(String const& basePath) const;
  [[nodiscard]] ByteArray read(String const& basePath) const;
  [[nodiscard]] ImageConstPtr readImage(String const& path) const;
  [[nodiscard]] ImageConstPtr applyImagePatches(ImageConstPtr image, String const& path, List<pair<String, AssetSourcePtr>> const& patches) const;

  [[nodiscard]] Json readJson(String const& basePath) const;
  [[nodiscard]] Json applyJsonPatches(Json const& input, String const& path, List<pair<String, AssetSourcePtr>> const& patches) const;
  [[nodiscard]] Json checkPatchArray(String const& path, AssetSourcePtr const& source, Json const& result, JsonArray const& patchData, Maybe<Json> const& external) const;
  ...
  [[nodiscard]] bool doLoad(AssetId const& id) const;
  [[nodiscard]] bool doPost(AssetId const& id) const;
  ...
  [[nodiscard]] shared_ptr<AssetData> loadAsset(AssetId const& id) const;

  [[nodiscard]] shared_ptr<AssetData> loadJson(AssetPath const& path) const;
  [[nodiscard]] shared_ptr<AssetData> loadImage(AssetPath const& path) const;
  [[nodiscard]] shared_ptr<AssetData> loadAudio(AssetPath const& path) const;
  [[nodiscard]] shared_ptr<AssetData> loadFont(AssetPath const& path) const;
  [[nodiscard]] shared_ptr<AssetData> loadBytes(AssetPath const& path) const;

  [[nodiscard]] shared_ptr<AssetData> postProcessAudio(shared_ptr<AssetData> const& original) const;
