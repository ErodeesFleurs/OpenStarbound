...
public:
  ...
  [[nodiscard]] bool stateTypeEnabled(String const& stateTypeName) const;

  // Returns the available states for the given state type.
  [[nodiscard]] StringList states(String const& stateTypeName) const;
  ...
  [[nodiscard]] bool setActiveState(String const& stateTypeName, String const& stateName, bool alwaysStart = false, bool reverse = false);

  // Restart this given state type's timer off at the beginning.
  void restartState(String const& stateTypeName);

  [[nodiscard]] ActiveStateInformation const& activeState(String const& stateTypeName) const;
  [[nodiscard]] ActivePartInformation const& activePart(String const& partName) const;
  [[nodiscard]] State const& getState(String const& stateTypeName, String const& stateName) const;

  [[nodiscard]] StringMap<Part> const& constParts() const;
  [[nodiscard]] StringMap<Part>& parts();
  ...
  [[nodiscard]] size_t activeStateIndex(String const& stateTypeName) const;
  [[nodiscard]] bool activeStateReverse(String const& stateTypeName) const;
  [[nodiscard]] bool setActiveStateIndex(String const& stateTypeName, size_t stateIndex, bool alwaysStart = false, bool reverse = false);
  ...
  [[nodiscard]] Json getStateFrameProperty(String const& stateType, String const& propertyName, String state, int frame) const;
  [[nodiscard]] Json getPartStateFrameProperty(String const& partName, String const& propertyName, String const& stateType, String state, int frame) const;

private:
  [[nodiscard]] static AnimationMode stringToAnimationMode(String const& string);
