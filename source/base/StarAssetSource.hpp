#pragma once

#include "StarIODevice.hpp"
#include "StarJson.hpp"

namespace Star {

class AssetSource;
using AssetSourcePtr = SharedPtr<AssetSource>;

struct AssetSourceExceptionTag { static constexpr char const* typeName = "AssetSourceException"; };
using AssetSourceException = TypedException<StarException, AssetSourceExceptionTag>;

// An asset source could be a directory on a filesystem, where assets are
// pulled directly from files, or a single pak-like file containing all assets,
// where assets are pulled from the correct region of the pak-like file.
class AssetSource {
public:
  virtual ~AssetSource() = default;

  // An asset source can have arbitrary metadata attached.
  [[nodiscard]] virtual JsonObject metadata() const = 0;

  // Should return all the available assets in this source
  [[nodiscard]] virtual StringList assetPaths() const = 0;

  // Open the given path in this source and return an IODevicePtr to it.
  [[nodiscard]] virtual IODevicePtr open(String const& path) = 0;

  // Read the entirety of the given path into a buffer.
  [[nodiscard]] virtual ByteArray read(String const& path) = 0;
};

}
