...
EnvironmentPainter::EnvironmentPainter(Ptr<Renderer> renderer) {
...
void EnvironmentPainter::renderStars(float pixelRatio, Vec2F const& screenSize, SkyRenderData const& sky) {
  float nightSkyAlpha = 1.0f - std::min(sky.dayLevel, sky.skyAlpha);
  ...
  auto stars = m_starGenerator->generate(field, [&](RandomSource& rand) -> std::pair<size_t, float> {
  ...
  if (sky.type == SkyType::Orbital || sky.type == SkyType::Warp) {
    ...
    for (size_t i = 0; i < debrisFields.size(); ++i) {
      ...
      for (Json const& json : imageOptions) {
        RefPtr<Texture> texture = m_textureGroup->loadTexture(*json.stringPtr());
        biggest = biggest.piecewiseMax(texture->size());
      }

      float screenBuffer = std::ceil((float)biggest.max() * (float)Constants::sqrt2);
      ...
      auto debrisItems = m_debrisGenerators[i]->generate(field,
                                                         [&](RandomSource& rand) -> std::pair<StringView, float> {
                                                           ...
                                                           return {debrisImage, debrisAngularVelocity};
                                                         });
      ...
      for (auto& debrisItem : debrisItems) {
        ...
        float debrisAngle = std::fmod(Constants::deg2rad * debrisItem.second.second * sky.epochTime, Constants::pi * 2) + sky.starRotation;
...
void EnvironmentPainter::renderPlanetHorizon(float pixelRatio, Vec2F const& screenSize, SkyRenderData const& sky) {
  ...
  for (auto const& layer : planetHorizon.layers) {
    RefPtr<Texture> leftTexture = m_textureGroup->loadTexture(layer.first);
    Vec2F leftTextureSize(leftTexture->size());
    RefPtr<Texture> rightTexture = m_textureGroup->loadTexture(layer.second);
...
void EnvironmentPainter::renderSky(Vec2F const& screenSize, SkyRenderData const& sky) {
  auto& primitives = m_renderer->immediatePrimitives();
  primitives.emplace_back(std::in_place_type_t<RenderQuad>(), RefPtr<Texture>(),
                          RenderVertex{.screenCoordinate = Vec2F(0, 0), .textureCoordinate = Vec2F(), .color = sky.bottomRectColor.toRgba(), .param1 = 0.0f},
                          RenderVertex{.screenCoordinate = Vec2F(screenSize[0], 0), .textureCoordinate = Vec2F(), .color = sky.bottomRectColor.toRgba(), .param1 = 0.0f},
                          RenderVertex{.screenCoordinate = screenSize, .textureCoordinate = Vec2F(), .color = sky.topRectColor.toRgba(), .param1 = 0.0f},
                          RenderVertex{.screenCoordinate = Vec2F(0, screenSize[1]), .textureCoordinate = Vec2F(), .color = sky.topRectColor.toRgba(), .param1 = 0.0f});
...
void EnvironmentPainter::renderParallaxLayers(
  ...
  for (auto& layer : layers) {
    ...
    if (layer.unlit || layer.lightMapped)
      drawColor = Vec4B(255, 255, 255, std::floor(255 * layer.alpha));
    else
      drawColor = Vec4B(sky.environmentLight.toRgb(), std::floor(255 * layer.alpha));
    ...
    if (layer.speed[0] != 0) {
      double drift = std::fmod((double)layer.speed[0] * (sky.epochTime / (double)sky.dayLength), (double)parallaxSize[0]);
      parallaxOffset[0] = std::fmod(parallaxOffset[0] + drift * camera.pixelRatio(), parallaxPixels[0]);
    }
    if (layer.speed[1] != 0) {
      double drift = std::fmod((double)layer.speed[1] * (sky.epochTime / (double)sky.dayLength), (double)parallaxSize[1]);
      parallaxOffset[1] = std::fmod(parallaxOffset[1] + drift * camera.pixelRatio(), parallaxPixels[1]);
    }
    ...
    int left = std::floor(parallaxScreenLeft + parallaxScreenOffset[0]);
    int bottom = std::floor(parallaxScreenBottom + parallaxScreenOffset[1]);
    int right = std::ceil(parallaxScreenLeft + parallaxScreenWidth + parallaxScreenOffset[0]);
    int top = std::ceil(parallaxScreenBottom + parallaxScreenHeight + parallaxScreenOffset[1]);
    ...
    for (int y = bottom; y <= top; ++y) {
      ...
      for (int x = left; x <= right; ++x) {
        ...
        for (auto const& textureImage : layer.textures) {
          ...
          if (layer.frameNumber > 1) {
            auto time_within_cycle = (float)std::fmod(sky.epochTime, (double)layer.animationCycle);
          ...
          if (auto texture = m_textureGroup->tryTexture(withDirectives)) {
            ...
            primitives.emplace_back(std::in_place_type_t<RenderQuad>(), std::move(texture),
                                    RenderVertex{.screenCoordinate = drawRect.min(), .textureCoordinate = subImage.min(), .color = drawColor, .param1 = lightMapMultiplier},
                                    RenderVertex{.screenCoordinate = {drawRect.xMax(), drawRect.yMin()}, .textureCoordinate = {subImage.xMax(), subImage.yMin()}, .color = drawColor, .param1 = lightMapMultiplier},
                                    RenderVertex{.screenCoordinate = drawRect.max(), .textureCoordinate = subImage.max(), .color = drawColor, .param1 = lightMapMultiplier},
                                    RenderVertex{.screenCoordinate = {drawRect.xMin(), drawRect.yMax()}, .textureCoordinate = {subImage.xMin(), subImage.yMax()}, .color = drawColor, .param1 = lightMapMultiplier});
          }
        }
      }
    }
...
void EnvironmentPainter::drawRay(float pixelRatio,
  ...
  m_renderer->immediatePrimitives().emplace_back(std::in_place_type_t<RenderQuad>(), RefPtr<Texture>(),
                                                 RenderVertex{.screenCoordinate = start + Vec2F(std::cos(angle + width), std::sin(angle + width)) * length, .textureCoordinate = {}, .color = Vec4B(rayColor, 0), .param1 = 0.0f},
                                                 RenderVertex{.screenCoordinate = start + Vec2F(std::cos(angle + width), std::sin(angle + width)) * SunRadius * sunScale * pixelRatio,
                                                              .textureCoordinate = {},
                                                              .color = Vec4B(rayColor,
                                                                             (int)(RayMinUnscaledAlpha + std::abs(m_rayPerlin.get(angle * 896 + time * 30) * RayUnscaledAlphaVariance))
                                                                               * sum
                                                                               * alpha),
                                                              .param1 = 0.0f},
                                                 RenderVertex{.screenCoordinate = start + Vec2F(std::cos(angle), std::sin(angle)) * SunRadius * sunScale * pixelRatio,
                                                              .textureCoordinate = {},
                                                              .color = Vec4B(rayColor,
                                                                             (int)(RayMinUnscaledAlpha + std::abs(m_rayPerlin.get(angle * 626 + time * 30) * RayUnscaledAlphaVariance))
                                                                               ...
                                                                               * alpha),
                                                              .param1 = 0.0f},
                                                 RenderVertex{.screenCoordinate = start + Vec2F(std::cos(angle), std::sin(angle)) * length, .textureCoordinate = {}, .color = Vec4B(rayColor, 0), .param1 = 0.0f});
}
...
void EnvironmentPainter::drawOrbiter(float pixelRatio, Vec2F const& screenSize, SkyRenderData const& sky, SkyOrbiter const& orbiter) {
  ...
  RefPtr<Texture> texture = m_textureGroup->loadTexture(orbiter.image);
...
auto EnvironmentPainter::starsHash(SkyRenderData const& sky, Vec2F const& viewSize) const -> uint64_t {
  XXHash64 hasher;
...
void EnvironmentPainter::setupStars(SkyRenderData const& sky) {
  ...
  m_starGenerator = make_shared<Random2dPointGenerator<std::pair<size_t, float>>>(sky.skyParameters.seed, starCellSize, starCount);
  ...
  for (size_t i = 0; i < debrisFields.size(); ++i) {
    ...
    m_debrisGenerators[i] = make_shared<Random2dPointGenerator<std::pair<String, float>, double>>(debrisSeed, debrisCellSize, debrisCountRange);
  }
}
