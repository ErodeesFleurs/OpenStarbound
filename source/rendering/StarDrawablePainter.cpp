...
DrawablePainter::DrawablePainter(Ptr<Renderer> renderer, Ptr<AssetTextureGroup> textureGroup) {
...
void DrawablePainter::drawDrawable(Drawable const& drawable) {
  ...
  if (auto linePart = drawable.part.ptr<Drawable::LinePart>()) {
    ...
    auto& primitive = primitives.emplace_back(std::in_place_type_t<RenderQuad>(),
                                              ...
                                              line.max() + left,
                                              color, fullbright);
    if (auto* endColor = &linePart->endColor.value()) {
      auto& quad = primitive.get<RenderQuad>();
    ...
    RefPtr<Texture> texture = m_textureGroup->loadTexture(imagePart->image);
...
void DrawablePainter::cleanup(std::int64_t textureTimeout) {
