...
auto ImagePartReader::size() const -> Vec2U {
  if (m_images.empty())
    return Vec2U{0, 0};
...
void ImagePartReader::forEachTile(TileCallback const& callback) const {
  for (auto const& entry : m_images) {
    ...
    ConstPtr<Image> const& image = entry.second;
...
auto connectorColorValue(Vec4B const& color) -> String {
  return strf("{},{},{},{}", color[0], color[1], color[2], color[3]);
}

auto variantMapToTile(JsonObject const& tile) -> Tile {
  Tile result;
...
auto ImageTileset::getTile(Vec4B color) const -> Tile const* {
  if (color[3] == 0)
...
auto ImageTileset::colorAsInt(Vec4B color) const -> unsigned {
