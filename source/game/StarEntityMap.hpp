...
public:
  ...
  [[nodiscard]] EntityId reserveEntityId();
  // Or a specific one, can fail.
  [[nodiscard]] Maybe<EntityId> maybeReserveEntityId(EntityId entityId);
  // If it doesn't matter that we don't get the one want
  [[nodiscard]] EntityId reserveEntityId(EntityId entityId);
  ...
  [[nodiscard]] EntityPtr removeEntity(EntityId entityId);

  [[nodiscard]] size_t size() const;
  [[nodiscard]] List<EntityId> entityIds() const;
  ...
  [[nodiscard]] EntityId uniqueEntityId(String const& uniqueId) const;

  [[nodiscard]] EntityPtr entity(EntityId entityId) const;
  [[nodiscard]] EntityPtr uniqueEntity(String const& uniqueId) const;

  // Queries entities based on metaBoundBox
  [[nodiscard]] List<EntityPtr> entityQuery(RectF const& boundBox, EntityFilter const& filter = {}) const;

  // A fuzzy query of the entities at this position, sorted by closeness.
  [[nodiscard]] List<EntityPtr> entitiesAt(Vec2F const& pos, EntityFilter const& filter = {}) const;

  [[nodiscard]] List<TileEntityPtr> entitiesAtTile(Vec2I const& pos, EntityFilterOf<TileEntity> const& filter = {}) const;

  // Sort of a fuzzy line intersection test.  Tests if a given line intersects
  // the bounding box of any entities, and returns them.
  [[nodiscard]] List<EntityPtr> entityLineQuery(Vec2F const& begin, Vec2F const& end, EntityFilter const& filter = {}) const;
  ...
  // caused it.
  [[nodiscard]] EntityPtr findEntity(RectF const& boundBox, EntityFilter const& filter) const;
  [[nodiscard]] EntityPtr findEntityLine(Vec2F const& begin, Vec2F const& end, EntityFilter const& filter) const;
  [[nodiscard]] EntityPtr findEntityAtTile(Vec2I const& pos, EntityFilterOf<TileEntity> const& filter) const;

  // Closest entity that satisfies the given selector, if given.
  [[nodiscard]] EntityPtr closestEntity(Vec2F const& center, float radius, EntityFilter const& filter = {}) const;

  // Returns interactive entity that is near the given world position
  [[nodiscard]] InteractiveEntityPtr interactiveEntityNear(Vec2F const& pos, float maxRadius = 1.5f) const;

  // Whether or not any tile entity occupies this tile
  [[nodiscard]] bool tileIsOccupied(Vec2I const& pos, bool includeEphemeral = false) const;

  // Intersects any entity's collision area
  [[nodiscard]] bool spaceIsOccupied(RectF const& rect, bool includeEphemeral = false) const;
  ...
  template <typename EntityT>
  [[nodiscard]] SharedPtr<EntityT> getUnique(String const& uniqueId) const;

  template <typename EntityT>
  [[nodiscard]] List<SharedPtr<EntityT>> query(RectF const& boundBox, EntityFilterOf<EntityT> const& filter = {}) const;

  template <typename EntityT>
  [[nodiscard]] List<SharedPtr<EntityT>> all(EntityFilterOf<EntityT> const& filter = {}) const;

  template <typename EntityT>
  [[nodiscard]] List<SharedPtr<EntityT>> lineQuery(Vec2F const& begin, Vec2F const& end, EntityFilterOf<EntityT> const& filter = {}) const;

  template <typename EntityT>
  [[nodiscard]] SharedPtr<EntityT> closest(Vec2F const& center, float radius, EntityFilterOf<EntityT> const& filter = {}) const;
