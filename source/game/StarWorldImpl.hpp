...
namespace Star::WorldImpl {
template <typename TileSectorArray>
auto tileIsOccupied(std::shared_ptr<TileSectorArray> const& tileSectorArray, Ptr<EntityMap> const& entityMap,
                    Vec2I const& pos, TileLayer layer, bool includeEphemeral, bool checkCollision = false) -> bool;

template <typename TileSectorArray>
auto tileCollisionKind(std::shared_ptr<TileSectorArray> const& tileSectorArray, Ptr<EntityMap> const& entityMap,
                       Vec2I const& pos) -> CollisionKind;

template <typename TileSectorArray>
auto rectTileCollision(std::shared_ptr<TileSectorArray> const& tileSectorArray, RectI const& region, bool solidCollision) -> bool;

template <typename TileSectorArray>
auto lineTileCollision(WorldGeometry const& worldGeometry, std::shared_ptr<TileSectorArray> const& tileSectorArray, Vec2F const& begin, Vec2F const& end, CollisionSet const& collisionSet) -> bool;

template <typename TileSectorArray>
auto lineTileCollisionPoint(WorldGeometry const& worldGeometry, std::shared_ptr<TileSectorArray> const& tileSectorArray, Vec2F const& begin, Vec2F const& end, CollisionSet const& collisionSet) -> std::optional<std::pair<Vec2F, Vec2I>>;

template <typename TileSectorArray>
auto collidingTilesAlongLine(WorldGeometry const& worldGeometry, std::shared_ptr<TileSectorArray> const& tileSectorArray,
                             Vec2F const& begin, Vec2F const& end, CollisionSet const& collisionSet, size_t maxSize, bool includeEdges) -> List<Vec2I>;

inline auto tileDamageParameters(WorldTile* tile, TileLayer layer, TileDamage const& tileDamage) -> TileDamageParameters;
template <typename TileSectorArray>
auto damageWouldDestroy(std::shared_ptr<TileSectorArray> const& tileSectorArray, Vec2I pos, TileLayer layer, TileDamage const& tileDamage) -> bool;

template <typename GetTileFunction>
auto canPlaceMaterial(Ptr<EntityMap> const& entityMap,
                      Vec2I const& pos, TileLayer layer, MaterialId material, bool allowEntityOverlap, bool allowTileOverlap, GetTileFunction& getTile) -> bool;
// returns true if this material could be placed if in the same batch other
// tiles can be placed
// that connect to it
template <typename GetTileFunction>
auto perhapsCanPlaceMaterial(Ptr<EntityMap> const& entityMap,
                             Vec2I const& pos, TileLayer layer, MaterialId material, bool allowEntityOverlap, bool allowTileOverlap, GetTileFunction& getTile) -> bool;
template <typename GetTileFunction>
auto canPlaceMaterialColorVariant(Vec2I const& pos, TileLayer layer, MaterialColorVariant color, GetTileFunction& getTile) -> bool;
template <typename GetTileFunction>
auto canPlaceMod(Vec2I const& pos, TileLayer layer, ModId mod, GetTileFunction& getTile) -> bool;
template <typename GetTileFunction>
auto validateTileModification(Ptr<EntityMap> const& entityMap, Vec2I const& pos, TileModification const& modification, bool allowEntityOverlap, GetTileFunction& getTile) -> std::pair<bool, bool>;
auto validateTileReplacement(TileModification const& modification) -> bool;
// Split modification list into good and bad
template <typename GetTileFunction>
auto splitTileModifications(Ptr<EntityMap> const& entityMap, TileModificationList const& modificationList,
                            bool allowEntityOverlap, GetTileFunction& getTile, std::function<bool(Vec2I pos, TileModification modification)> extraCheck = {}) -> std::pair<TileModificationList, TileModificationList>;

template <typename TileSectorArray>
auto windLevel(std::shared_ptr<TileSectorArray> const& tileSectorArray, Vec2F const& position, float weatherWindLevel) -> float;

template <typename TileSectorArray>
auto temperature(std::shared_ptr<TileSectorArray> const& tileSectorArray, ConstPtr<WorldTemplate> const& worldTemplate,
                 ConstPtr<Sky> const& sky, Vec2F const& pos) -> float;

template <typename TileSectorArray>
auto breathable(World const* world, std::shared_ptr<TileSectorArray> const& tileSectorArray, ConstPtr<WorldTemplate> const& worldTemplate, Vec2F const& pos) -> bool;

template <typename TileSectorArray>
auto lightLevel(std::shared_ptr<TileSectorArray> const& tileSectorArray, Ptr<EntityMap> const& entityMap, WorldGeometry const& worldGeometry,
                ConstPtr<WorldTemplate> const& worldTemplate, ConstPtr<Sky> const& sky, CellularLightIntensityCalculator& lighting, Vec2F pos) -> float;

auto getInteractiveInRange(WorldGeometry const& geometry, Ptr<EntityMap> const& entityMap,
                           Vec2F const& targetPosition, Vec2F const& sourcePosition, float maxRange) -> Ptr<InteractiveEntity>;
auto isTileEntityInRange(WorldGeometry const& geometry, Ptr<EntityMap> const& entityMap,
                         EntityId targetEntity, Vec2F const& sourcePosition, float maxRange) -> bool;
template <typename TileSectorArray>
auto canReachEntity(WorldGeometry const& geometry, std::shared_ptr<TileSectorArray> const& tileSectorArray,
                    Ptr<EntityMap> const& entityMap, Vec2F const& sourcePosition, float maxRange, EntityId targetEntity, bool preferInteractive = true) -> bool;

template <typename TileSectorArray>
auto tileIsOccupied(std::shared_ptr<TileSectorArray> const& tileSectorArray, Ptr<EntityMap> const& entityMap,
                    Vec2I const& pos, TileLayer layer, bool includeEphemeral, bool checkCollision) -> bool {
...
template <typename TileSectorArray>
auto tileCollisionKind(std::shared_ptr<TileSectorArray> const& tileSectorArray, Ptr<EntityMap> const&,
                       Vec2I const& pos) -> CollisionKind {
  return tileSectorArray->tile(pos).getCollision();
}

template <typename TileSectorArray>
auto rectTileCollision(std::shared_ptr<TileSectorArray> const& tileSectorArray, RectI const& region, CollisionSet const& collisionSet) -> bool {
  return tileSectorArray->tileSatisfies(region, [&collisionSet](Vec2I const&, typename TileSectorArray::Tile const& tile) -> auto {
    return isColliding(tile.getCollision(), collisionSet);
  });
}

template <typename TileSectorArray>
auto lineTileCollision(WorldGeometry const& worldGeometry, std::shared_ptr<TileSectorArray> const& tileSectorArray,
                       Vec2F const& begin, Vec2F const& end, CollisionSet const& collisionSet) -> bool {
...
template <typename TileSectorArray>
auto lineTileCollisionPoint(WorldGeometry const& worldGeometry, std::shared_ptr<TileSectorArray> const& tileSectorArray,
                            Vec2F const& begin, Vec2F const& end, CollisionSet const& collisionSet) -> std::optional<std::pair<Vec2F, Vec2I>> {
  ...
  auto blockPosition = Vec2F(*collidingBlock);
  ...
  if (direction[0] * blockDistance[0] < 0)
    blockDistance[0] = 0.0f;
  if (direction[1] * blockDistance[1] < 0)
    blockDistance[1] = 0.0f;

  // line length per block in each axis, don't divide by 0
  float deltaDistX = direction[0] == 0 ? 0.0f : 1.0f / std::abs(direction[0]);
  float deltaDistY = direction[1] == 0 ? 0.0f : 1.0f / std::abs(direction[1]);

  // distance to each edge of the intersect
  Vec2F intersectDist = Vec2F(std::abs(blockDistance[0]) * deltaDistX, std::abs(blockDistance[1]) * deltaDistY);

  Vec2I normal = Vec2I(intersectDist[0] > intersectDist[1] ? 1 : 0, intersectDist[1] > intersectDist[0] ? 1 : 0);
  normal[0] *= std::copysign(1.0f, direction[0]);
  normal[1] *= std::copysign(1.0f, direction[1]);
  Vec2F position = begin + (direction * std::max(intersectDist[0], intersectDist[1]));
  ...
  return std::pair<Vec2F, Vec2I>(position, normal);
}

template <typename TileSectorArray>
inline auto liquidLevel(std::shared_ptr<TileSectorArray> const& tileSectorArray, RectF const& region) -> LiquidLevel {
  if (region.isEmpty())
    return {};
  ...
  tileSectorArray->tileEach(sampleRect, [&region, &totalSpace, &totals](Vec2I const& pos, typename TileSectorArray::Tile const& tile) -> auto {
  ...
  return {maximumLiquidId, totalLiquidLevel / totalSpace};
}
...
auto collidingTilesAlongLine(WorldGeometry const& worldGeometry, std::shared_ptr<TileSectorArray> const& tileSectorArray,
                             Vec2F const& begin, Vec2F const& end, CollisionSet const& collisionSet, size_t maxSize, bool includeEdges) -> List<Vec2I> {
  List<Vec2I> res;
  forBlocksAlongLine<float>(begin, worldGeometry.diff(end, begin), [&](int x, int y) -> auto {
    if (res.size() >= maxSize)
      return false;
    if (tileSectorArray->tile({x, y}).isColliding(collisionSet))
      res.append({x, y});
  ...
  while (tileIt.hasNext()) {
    ...
    if (any(lineSet, [&](Line2F const& line) -> auto { return tileRect.edgeIntersection(line).glances; }))
      tileIt.remove();
  }
  return res;
}
...
auto canPlaceMaterial(Ptr<EntityMap> const& entityMap,
  ...
  auto isAdjacentToConnectable = [&](Vec2I const& pos, int distance, bool foreground) -> auto {
    if (pos.y() - distance < 0)
      return true;
...
inline auto validateTileReplacement(TileModification const& modification) -> bool {
  if (auto placeMaterial = modification.ptr<PlaceMaterial>()) {
...
template <typename TileSectorArray>
auto breathable(World const* world, std::shared_ptr<TileSectorArray> const& tileSectorArray, HashMap<DungeonId, bool> const& breathableMap,
                ConstPtr<WorldTemplate> const& worldTemplate, Vec2F const& pos) -> bool {
...
auto lightLevel(std::shared_ptr<TileSectorArray> const& tileSectorArray, Ptr<EntityMap> const& entityMap, WorldGeometry const& worldGeometry,
  ...
  // sector size.
  std::array<CellularLightIntensityCalculator::Cell, WorldSectorSize> lightingCellColumn;
  tileSectorArray->tileEvalColumns(lighting.calculationRegion(), [&](Vec2I const& pos, typename TileSectorArray::Tile const* column, size_t ySize) -> auto {
    ...
    lighting.setCellColumn(pos, lightingCellColumn.data(), ySize);
  });
...
inline auto getInteractiveInRange(WorldGeometry const& geometry, Ptr<EntityMap> const& entityMap,
                                  Vec2F const& targetPosition, Vec2F const& sourcePosition, float maxRange) -> Ptr<InteractiveEntity> {
...
inline auto isTileEntityInRange(WorldGeometry const& geometry, Ptr<EntityMap> const& entityMap, EntityId targetEntity,
                                Vec2F const& sourcePosition, float maxRange) -> bool {
...
template <typename TileSectorArray>
auto canReachEntity(WorldGeometry const& geometry, std::shared_ptr<TileSectorArray> const& tileSectorArray,
                    Ptr<EntityMap> const& entityMap, Vec2F const& sourcePosition, float maxRange, EntityId targetEntity, bool preferInteractive) -> bool {
