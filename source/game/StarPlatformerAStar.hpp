...
namespace PlatformerAStar {
  ...
  public:
    ...
    [[nodiscard]] Maybe<Path> const& result() const;
  ...
  private:
    ...
    [[nodiscard]] float heuristicCost(Vec2F const& fromPosition, Vec2F const& toPosition) const;
    [[nodiscard]] Edge defaultCostEdge(Action action, Node const& source, Node const& target) const;
    ...
    [[nodiscard]] Vec2F acceleration(Vec2F pos) const;
    [[nodiscard]] Vec2F simulateArcCollision(Vec2F position, Vec2F velocity, float dt, bool& collidedX, bool& collidedY) const;
    void simulateArc(Node const& node, function<void(Node, bool)> func) const;

    [[nodiscard]] bool validPosition(Vec2F pos, BoundBoxKind boundKind = BoundBoxKind::Full) const;
    ...
    [[nodiscard]] bool inLiquid(Vec2F pos) const;

    [[nodiscard]] RectF boundBox(Vec2F pos, BoundBoxKind boundKind = BoundBoxKind::Full) const;
    // Returns a rect that covers the tiles below the entity's feet if it was at
    // pos
    [[nodiscard]] RectI groundCollisionRect(Vec2F pos, BoundBoxKind boundKind) const;
    // Returns a rect that covers a 1 tile wide space below the entity's feet at
    // node pos
    [[nodiscard]] Vec2I groundNodePosition(Vec2F pos) const;
