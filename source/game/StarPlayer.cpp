...
Player::Player(PlayerConfigPtr config, Uuid uuid, AssetsConstPtr assets, ConfigurationPtr configuration, MaterialDatabaseConstPtr materialDatabase, ItemDatabaseConstPtr itemDatabase, ObjectDatabaseConstPtr objectDatabase, QuestTemplateDatabaseConstPtr questTemplateDatabase, VersioningDatabaseConstPtr versioningDatabase, CodexDatabaseConstPtr codexDatabase, DanceDatabaseConstPtr danceDatabase, EmoteProcessorConstPtr emoteProcessor, RadioMessageDatabaseConstPtr radioMessageDatabase, AiDatabaseConstPtr aiDatabase, CollectionDatabaseConstPtr collectionDatabase, SpeciesDatabaseConstPtr speciesDatabase, EntityFactoryConstPtr entityFactory, LiquidsDatabaseConstPtr liquidsDatabase, TechDatabaseConstPtr techDatabase, StatusEffectDatabaseConstPtr statusEffectDatabase, ParticleDatabaseConstPtr particleDatabase, ImageMetadataDatabaseConstPtr imageMetadataDatabase)
  ...
  m_walkIntoInteractBias = jsonToVec2F(m_assets->json("/player.config:walkIntoInteractBias"));
  ...
  m_foodLowThreshold = m_assets->json("/player.config:foodLowThreshold").toFloat();
  m_foodLowStatusEffects = m_assets->json("/player.config:foodLowStatusEffects").toArray().transformed(jsonToPersistentStatusEffect);
  m_foodEmptyStatusEffects = m_assets->json("/player.config:foodEmptyStatusEffects").toArray().transformed(jsonToPersistentStatusEffect);

  m_narrativeQueue->init(m_assets->json("/player.config:inCinematicStatusEffects").toArray().transformed(jsonToPersistentStatusEffect));
...
void Player::render(RenderCallback* renderCallback) {
  ...
  if (!footstepSound.empty() && !m_techController->parentState() && !m_techController->parentHidden()) {
    auto footstepAudio = m_assets->audio(footstepSound);
...
void Player::giveItem(ItemDescriptor const& descriptor) {
  giveItem(m_itemDatabase->item(descriptor));
}
...
bool Player::addBlueprint(ItemDescriptor const& descriptor, bool showFailure) {
  ...
  if (!m_blueprints->isKnown(descriptor)) {
    ...
    queueUIMessage(m_assets->json("/player.config:blueprintAlreadyKnown").toString().replace("<ItemName>", item->friendlyName()));
  }
...
void Player::setModeType(PlayerMode mode) {
  ...
  m_modeConfig = PlayerModeConfig(m_assets->json("/playermodes.config").get(PlayerModeNames.getRight(mode)));
}
...
void Player::finalizeCreation() {
  ...
  for (auto const& descriptor : m_speciesDatabase->species(m_appearance.m_identity.species)->defaultItems())
    m_inventory->addItems(itemDatabase->item(descriptor));

  for (auto const& descriptor : m_config->defaultBlueprints)
    m_blueprints->add(descriptor);

  for (auto const& descriptor : m_speciesDatabase->species(m_appearance.m_identity.species)->defaultBlueprints())
    m_blueprints->add(descriptor);
