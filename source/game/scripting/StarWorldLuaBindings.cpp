...
static WorldServer* requireWorldServer(World& world, String const& operation) {
  if (auto worldServer = as<WorldServer>(&world))
    return worldServer;
  throw StarException(strf("{} requires a server world", operation));
}
...
LuaCallbacks makeWorldCallbacks(World& world) {
  ...
  if (auto serverWorld = as<WorldServer>(&world)) {
    ...
    callbacks.registerCallback("enqueuePlacement", [serverWorld](List<Json> distributionConfigs, Maybe<DungeonId> id) {
      ...
      auto plantDatabase = serverWorld->plantDatabase();
      auto distributions = distributionConfigs.transformed([assets, plantDatabase](Json const& config) {
        return BiomeItemDistribution(assets, plantDatabase, config, Random::randu64());
      });
    ...
    callbacks.registerCallback("setTemplate", [serverWorld](Json worldTemplate) {
      auto newTemplate = make_shared<WorldTemplate>(serverWorld->assets(), serverWorld->terrainDatabase(), serverWorld->biomeDatabase(), worldTemplate, serverWorld->dungeonDefinitions());
...
Maybe<EntityId> WorldCallbacks::spawnMonster(
  ...
  try {
    auto monsterDatabase = requireWorldServer(world, "spawnMonster")->monsterDatabase();
...
Maybe<EntityId> WorldCallbacks::spawnNpc(World& world,
  ...
  try {
    auto npcDatabase = requireWorldServer(world, "spawnNpc")->npcDatabase();
...
Maybe<EntityId> WorldCallbacks::spawnStagehand(
  ...
  try {
    auto stagehandDatabase = requireWorldServer(world, "spawnStagehand")->stagehandDatabase();
...
Maybe<EntityId> WorldCallbacks::spawnProjectile(World& world,
  ...
  try {
    auto projectile = world.projectileDatabase()->createProjectile(projectileType, projectileParameters ? projectileParameters : JsonObject());
...
Maybe<EntityId> WorldCallbacks::spawnVehicle(
  ...
  auto vehicleDatabase = requireWorldServer(world, "spawnVehicle")->vehicleDatabase();
