...
LuaCallbacks LuaBindings::makeCelestialCallbacks(UniverseClient& client, BiomeDatabaseConstPtr biomeDatabase) {
  ...
  auto liquidsDatabase = client.liquidsDatabase();
  ...
  callbacks.registerCallback("worldImages", [celestialDatabase, assets, liquidsDatabase](Json const& coords) -> List<pair<String, float>> {
    CelestialCoordinate coordinate = CelestialCoordinate(coords);
    return CelestialGraphics::drawWorld(celestialDatabase, coordinate, assets, liquidsDatabase);
  });
