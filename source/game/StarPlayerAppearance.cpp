...
void PlayerAppearance::init() {
  ...
  m_netHumanoid.setElementFactory([this]() {
    return make_shared<NetHumanoid>(HumanoidIdentity(), JsonObject(), Json(), m_player.m_assets, m_player.m_imageMetadataDatabase, m_player.m_speciesDatabase, m_player.m_danceDatabase, m_player.m_particleDatabase);
  });
  m_netHumanoid.addNetElement(make_shared<NetHumanoid>(m_identity, m_humanoidParameters, Json(), m_player.m_assets, m_player.m_imageMetadataDatabase, m_player.m_speciesDatabase, m_player.m_danceDatabase, m_player.m_particleDatabase));
}
...
HumanoidPtr PlayerAppearance::humanoid() {
  if (m_netHumanoid.netElements().empty())
    return {};
...
void PlayerAppearance::refreshHumanoidParameters() {
  ...
  if (m_player.inWorld()) {
    ...
    if (m_player.world()->isClient() && m_player.m_scriptedAnimator.initialized()) {
      ...
      m_player.m_scriptedAnimator.addCallbacks("animationConfig", LuaBindings::makeScriptedAnimatorCallbacks(*humanoid()->networkedAnimator(), [this](String const& name, Json const& defaultValue) -> Json {
...
void PlayerAppearance::setBodyDirectives(String const& directives) {
  m_identity.bodyDirectives = directives;
  updateIdentity();
}
void PlayerAppearance::setEmoteDirectives(String const& directives) {
  m_identity.emoteDirectives = directives;
  updateIdentity();
}

void PlayerAppearance::setHairGroup(String const& group) {
  m_identity.hairGroup = group;
  updateIdentity();
}
void PlayerAppearance::setHairType(String const& type) {
  m_identity.hairType = type;
  updateIdentity();
}
void PlayerAppearance::setHairDirectives(String const& directives) {
  m_identity.hairDirectives = directives;
  updateIdentity();
}

void PlayerAppearance::setFacialHairGroup(String const& group) {
  m_identity.facialHairGroup = group;
  updateIdentity();
}
void PlayerAppearance::setFacialHairType(String const& type) {
  m_identity.facialHairType = type;
  updateIdentity();
}
void PlayerAppearance::setFacialHairDirectives(String const& directives) {
  m_identity.facialHairDirectives = directives;
  updateIdentity();
}

void PlayerAppearance::setFacialMaskGroup(String const& group) {
  m_identity.facialMaskGroup = group;
  updateIdentity();
}
void PlayerAppearance::setFacialMaskType(String const& type) {
  m_identity.facialMaskType = type;
  updateIdentity();
}
void PlayerAppearance::setFacialMaskDirectives(String const& directives) {
