...
auto Tenant::criteriaSatisfied(StringMap<unsigned> const& colonyTags) const -> bool {
  // Check whether colonyTags is a supermultiset of colonyTagCriteria
  for (std::pair<String, unsigned> const& entry : colonyTagCriteria.pairs()) {
...
auto TenantDatabase::getTenant(String const& name) const -> Ptr<Tenant> {
  MutexLocker locker(m_cacheMutex);
  return m_tenantCache.get(name,
                           [this](String const& name) -> Ptr<Tenant> {
...
auto TenantDatabase::getMatchingTenants(StringMap<unsigned> const& colonyTags) const -> List<Ptr<Tenant>> {
  ...
  List<Ptr<Tenant>> matchingTenants;
  for (String const& name : m_paths.keys()) {
    Ptr<Tenant> tenant = getTenant(name);
...
auto TenantDatabase::readTenant(String const& path) -> Ptr<Tenant> {
  try {
    auto assets = Root::singleton().assets();
    ...
    for (std::pair<String, Json> const& entry : config.getObject("colonyTagCriteria").pairs()) {
    ...
    List<TenantSpawnable> tenants = config.getArray("tenants").transformed([](Json const& json) -> TenantSpawnable {
      ...
      if (spawn == "monster") {
        return TenantMonsterSpawnable{.type = json.getString("type"), .level = json.optFloat("level"), .overrides = json.opt("overrides")};
      } else {

        Json speciesJson = json.get("species");
        List<String> species;
        if (speciesJson.isType(Json::Type::Array))
          species = speciesJson.toArray().transformed(std::mem_fn(&Json::toString));
        else
          species.append(speciesJson.toString());

        return TenantNpcSpawnable{.species = species, .type = json.getString("type"), .level = json.optFloat("level"), .overrides = json.opt("overrides")};
      }
    });

    std::optional<TenantRent> rent = config.opt("rent").transform([](Json const& json) -> TenantRent {
      return TenantRent{.periodRange = jsonToVec2F(json.get("periodRange")), .pool = json.getString("pool")};
    });

    return std::make_shared<Tenant>(Tenant{.name = name, .priority = priority, .colonyTagCriteria = colonyTagCriteria, .tenants = tenants, .rent = rent, .config = config});
  } catch (std::exception const& e) {
