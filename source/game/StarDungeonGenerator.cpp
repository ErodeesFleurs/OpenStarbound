...
bool Part::allowsPlacement(int currentPlacementCount) const {
  for (auto const& rule : m_rules)
    if (!rule->allowSpawnCount(currentPlacementCount))
      return false;
  return true;
}
...
bool Part::doesNotConnectTo(Part const& part) const {
  for (auto const& rule : m_rules)
    if (rule->doesNotConnectToPart(part.name()))
      return true;
  for (auto const& rule : part.m_rules)
    if (rule->doesNotConnectToPart(m_name))
      return true;
  return false;
}
...
bool Part::checkPartCombinationsAllowed(StringMap<int> const& placementCounter) const {
  for (auto const& rule : m_rules)
    if (!rule->checkPartCombinationsAllowed(placementCounter))
      return false;
  return true;
}
...
void Part::scanAnchor() {
  ...
  highestGround = max(highestGround, highestLiquid);
  if (highestGround == -1)
...
DungeonDefinitions::DungeonDefinitions(AssetsConstPtr assets, TilesetDatabaseConstPtr tilesetDatabase) : m_assets(requireServiceValueAs<DungeonException>(std::move(assets), "DungeonDefinitions", "assets")), m_tilesetDatabase(requireServiceValueAs<DungeonException>(std::move(tilesetDatabase), "DungeonDefinitions", "tileset database")), m_definitionCache(DefinitionsCacheSize) {
